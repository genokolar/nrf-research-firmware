...
with open(path) as f:
  ...
  payloads[0] = payloads[0][0:2] + bytes(chr((payloads[0][2] + 1)), 'utf8') + bytes(chr((payloads[0][3] - 1)), 'utf8') + payloads[0][5:]
...
for x in range(0, 0x70, 2):
  response = dongle.send_command(0x21, 0x09, 0x0200, 0x0000, b"\x30" + bytes(chr(x), 'utf8') + b"\x00\x01" + b"\x00"*28)
